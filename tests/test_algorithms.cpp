...
TEST(AlgorithmTest, Initialization) {
    ...
    Algorithm algorithm(make_shared<POMDPAction>(XAction), 1, 10, 1);
...
TEST(AlgorithmTest, FindChildWithCState) {
    auto algorithm = make_shared<Algorithm>(make_shared<POMDPAction>(XAction), 1, 10, 1);

    EXPECT_FALSE(algorithm->exist_child_with_cstate(0));
    EXPECT_FALSE(algorithm->exist_child_with_cstate(1));

    auto  child1 = make_shared<Algorithm>(make_shared<POMDPAction>(XAction), 0, 10, 2);
    algorithm->children.push_back(child1);
    EXPECT_TRUE(algorithm->exist_child_with_cstate(0));
    EXPECT_FALSE(algorithm->exist_child_with_cstate(1));

    auto  child2 = make_shared<Algorithm>(make_shared<POMDPAction>(XAction), 1, 10, 2);
...
TEST(AlgorithmTest, EqualAlgorithm) {
    ...
    auto algorithm2 = make_shared<Algorithm>(make_shared<POMDPAction>(XAction), 1, 10, 1);
    EXPECT_EQ(*algorithm2, *algorithm);


    auto algorithm3 = make_shared<Algorithm>(make_shared<POMDPAction>(XAction), 0, 10, 1);
...
TEST(AlgorithmTest, NotEqualAlgorithm) {
    {
        ...
        Algorithm algorithm2(make_shared<POMDPAction>(XAction), 0, 10, 1);
        EXPECT_FALSE(algorithm2 == algorithm);
    }

    {
        Algorithm algorithm(make_shared<POMDPAction>(XAction), 1, 10, 1);
        Algorithm algorithm2(make_shared<POMDPAction>(XAction), 0, 10, 2);
        EXPECT_FALSE(algorithm2 == algorithm);
    }

    {
        Algorithm algorithm(make_shared<POMDPAction>(XAction), 1, 10, 1);
        Algorithm algorithm2(make_shared<POMDPAction>(HAction), 1, 10, 1);
        EXPECT_FALSE(algorithm2 == algorithm);
    }
    ...
    Algorithm algorithm3(make_shared<POMDPAction>(XAction), 0, 10, 1);
    Algorithm algorithm4(make_shared<POMDPAction>(HAction), 0, 10, 1);
    algorithm.children.push_back(make_shared<Algorithm>(algorithm));
    algorithm2.children.push_back(make_shared<Algorithm>(algorithm));
...
TEST(AlgorithmTest, HasMeas) {
...

    {
        ...
        Algorithm algorithm(make_shared<POMDPAction>(MeasAction), 1, 10, 1);
...
TEST(AlgorithmTest, HasCInstruction) {
...

    {
        ...
        Algorithm algorithm(make_shared<POMDPAction>(WriteAction), 1, 10, 1);
...
TEST(AlgorithmTest, SuccessorWithCState) {

    {
        ...
        Algorithm algorithm(make_shared<POMDPAction>(meas_action), 0, 10, 1);
...
TEST(AlgorithmTest, GetAlgFromList) {
    ...
    vector<shared_ptr<Algorithm>> algorithms = {make_shared<Algorithm>(algorithm), make_shared<Algorithm>(algorithm2)};

    EXPECT_EQ(get_algorithm_from_list(algorithms, make_shared<Algorithm>(algorithm)), 0);
    EXPECT_EQ(get_algorithm_from_list(algorithms, make_shared<Algorithm>(algorithm2)), 1);

    Algorithm algorithm3(make_shared<POMDPAction>(HAction), 2, 10, 1);
    EXPECT_EQ(get_algorithm_from_list(algorithms, make_shared<Algorithm>(algorithm3)), -1);
}

TEST(AlgorithmTest, DeepCopy) {
    Algorithm algorithm(make_shared<POMDPAction>(XAction), 1, 10, 1);
    auto algorithm2 = deep_copy_algorithm(make_shared<Algorithm>(algorithm));
...
TEST(AlgorithmTest, EndNodes) {
...

    {
        ...
        get_algorithm_end_nodes(make_shared<Algorithm>(algorithm), end_nodes);
        EXPECT_EQ(end_nodes.size(), 1);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(algorithm)), 0);
    }

    Algorithm child1(make_shared<POMDPAction>(XAction), 0, 10, 2);
    algorithm.children.push_back(make_shared<Algorithm>(child1));

    {
        vector<shared_ptr<Algorithm>> end_nodes;
        get_algorithm_end_nodes(make_shared<Algorithm>(algorithm), end_nodes);
        EXPECT_EQ(end_nodes.size(), 1);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(algorithm)), -1);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(child1)), 0);
    }

    Algorithm child2(make_shared<POMDPAction>(HAction), 1, 10, 2);
    algorithm.children.push_back(make_shared<Algorithm>(child2));
    {
        vector<shared_ptr<Algorithm>> end_nodes;
        get_algorithm_end_nodes(make_shared<Algorithm>(algorithm), end_nodes);
        ...
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(algorithm)), -1);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(child1)), 0);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(child2)), 1);
    }

    vector<Instruction> ins_seq({Instruction(GateName::Meas, 0, 0)});
    POMDPAction measAction("m1", ins_seq, 10, ins_seq);
    Algorithm meas_alg(make_shared<POMDPAction>(measAction), 0, 10, 1);
    Algorithm algorithm_(make_shared<POMDPAction>(XAction), 0, 10, 1);
    meas_alg.children.push_back(make_shared<Algorithm>(algorithm_));
    {
        vector<shared_ptr<Algorithm>> end_nodes;
        get_algorithm_end_nodes(make_shared<Algorithm>(meas_alg), end_nodes);
        EXPECT_EQ(end_nodes.size(), 2);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(meas_alg)), 0);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(algorithm_)), 1);
    }

    Algorithm meas_alg2(make_shared<POMDPAction>(measAction), 1, 10, 1);
    Algorithm algorithm__(make_shared<POMDPAction>(XAction), 1, 10, 1);
    meas_alg2.children.push_back(make_shared<Algorithm>(algorithm__));
    {
        vector<shared_ptr<Algorithm>> end_nodes;
        get_algorithm_end_nodes(make_shared<Algorithm>(meas_alg2), end_nodes);
        EXPECT_EQ(end_nodes.size(), 1);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(meas_alg2)), -1);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(algorithm__)), 0);
    }

    meas_alg.children.push_back(make_shared<Algorithm>(child2));
    {
        vector<shared_ptr<Algorithm>> end_nodes;
        get_algorithm_end_nodes(make_shared<Algorithm>(meas_alg), end_nodes);
        EXPECT_EQ(end_nodes.size(), 2);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(algorithm_)), 0);
        EXPECT_EQ(get_algorithm_from_list(end_nodes, make_shared<Algorithm>(child2)), 1);
    }
}
...
TEST(AlgorithmTest, MixedAlgorithm) {
    ...
    auto algorithm = make_shared<Algorithm>(make_shared<POMDPAction>(XAction), 1, 10, 1);
    auto algorithm2 = make_shared<Algorithm>(make_shared<POMDPAction>(HAction), 1, 10, 1);
    ...
    auto new_head = make_shared<Algorithm>(make_shared<POMDPAction>(random_branch), initial_classical_state, -1);
