...
int main(int argc, char* argv[]) {
    ...
    std::shared_ptr<antlr4::tree::ParseTree> prec_tree(raw_prec, [](auto*){ /* do nothing */ });
    auto *precondition = dynamic_cast<PreconditionAssertionParser::Precon_assertionContext*>(prec_tree.get());
    ...
    // vector<shared_ptr<Ensemble<double>>> initial_ensembles = get_initial_ensembles(precondition);
    // MarkovChain mc(hardware_specification, embedding);
    // max_depth = mc.get_depth(make_shared<Configuration>(program, initial_ensembles[0])); // the depth is independent of the initial ensemble (FIX this!)
    //
